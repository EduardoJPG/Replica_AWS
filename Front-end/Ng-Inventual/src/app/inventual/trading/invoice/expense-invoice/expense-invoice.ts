...
import {
  expenseInvoiceData,
  ExpenseInvoiceInterfaceData,
} from '../../../data/expenseInvoiceData';
...
@Component({
  ...
  templateUrl: './expense-invoice.html',
  styleUrl: './expense-invoice.scss',
...
export class ExpenseInvoice {
  ...
  calculateTotalDiscount() {
    this.totalDiscount = this.dataSource.data.reduce(
      (acc, product) => acc + (product.discount / 100) * product.amount,
      0
    );
  }
  ...
  calculateTotalTax() {
    this.totalTax = this.dataSource.data.reduce(
      (acc, product) => acc + (product.tax / 100) * product.amount,
      0
    );
  }
